...
| Staging stack is down and stale               | Mini-PC checkout is at DEV-97, about 10 PRs behind main; `docker compose ps` is empty; images are 9 days old                                                                                          | Everything demoed must first be redeployed. This is exactly DEV-47 (To Do, Fathema).                                                                                                                                                                                                         |
...
| Voice transcription end-to-end                | Code and tests pass; weights present on the box (1.1 GB, deployed Jul 13)                                                                                                                             | Not exercised since the stale deploy. Run `scripts/smoke-transcribe.sh` with an operator token after redeploy.                                                                                                                                                                               |
...
1. `git pull --ff-only` to current main. Never `reset --hard`.
2. `docker compose build` (or pull CI images), then `docker compose up -d`.
3. Run migrations and seed from the deployed images (DEV-97 made the published images able to do
   this) or from the checkout: `npm run db:migrate` and `npm run db:seed` in
   `services/core-api`.

...
A frozen or down Audit Service does not stop operators from working.
...
outbox row whose event was delivered _after_ that point still has `processed_at` set in `core_db`
...
| ---------- | ------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-08-07 | DEV-145 | Simulated (test-only trigger disable + `this_hash` overwrite on the newest row, seq 5, to reproduce a `this_hash` mismatch) | `audit_db` dump taken at seq 3 (before 2 further events landed and before the row was corrupted) | 5 of 5 (3 already-present rows, over-reset on purpose to prove the dedupe/no-op guarantee in section 5; 2 truly-missing rows, proving the under-reset recovery) | ~90 seconds from `docker compose stop audit` through outbox fully drained (excludes the setup/seeding time before the incident) | Ran in an isolated compose project (`mi-dev145-drill`) on dev staging, torn down after (`down -v` + worktree removed). Confirmed section 2.1's crash-loop log fields exactly (`brokenAtSeq: 5`, matching `reason` text, `checked: 4`). Confirmed post-recovery: `audit_events` had exactly 5 rows, no duplicates for the 3 over-reset rows, and the 2 previously-lost rows reappeared with the correct `resource_id`. Two setup gotchas found, not part of the recovery procedure itself but needed to stand the isolated project up: (1) `services/audit/db/migrate.ts` rewrites `@postgres:` to `@localhost:` in `AUDIT_MIGRATOR_DB_URL`, which breaks when the migration is run _inside_ a container instead of from the host; set `DB_HOST_LOCAL=postgres` to suppress the rewrite when migrating from inside the network. (2) The Audit Service's startup also requires reaching Azurite (`ensureReportsContainer`) before it will serve, unrelated to chain verification — an isolated drill project needs `azurite` running too, not just `postgres` and `audit`. |

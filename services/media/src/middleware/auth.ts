...
// JWT verification for the Media Service. A near-copy of core-api's middleware/auth.ts: services
// do not import across service directories (CLAUDE.md 8), so each service carries its own gate.
// The photo upload is called by the operator PWA with an Entra access token; identity comes from
...
// Marks a preHandler as one that authenticates the request. The onRoute fail-closed guard
// (auth-route-guard.ts) reads this symbol to confirm every non-public route is gated. Symbol.for
...
const verifyTokenImpl = async (req: FastifyRequest, _reply: FastifyReply): Promise<void> => {
  ...
  try {
    // ENTRA_* are read live (not from the cached config) so tests can toggle them per case. When
    // set, the token's issuer and audience must match the tenant and client id (ADR 0012); when
    // blank, the dev-token fallback applies and those checks are skipped.

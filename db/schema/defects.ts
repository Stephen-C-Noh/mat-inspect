...
export const defects = pgTable(
  ...
  (table) => [
    // Supports the readiness blocking-defect query: WHERE equipment_id = ? AND severity =
    // 'BLOCKING' AND status NOT IN ('RESOLVED', 'REJECTED') (ADR 0006).
    index('defects_equipment_severity_status_idx').on(
      table.equipmentId,
      table.severity,
      table.status,
    ),
